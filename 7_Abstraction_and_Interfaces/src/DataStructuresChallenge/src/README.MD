...
├── previous : ListItem
├── next() : ListItem
├── setNext(ListItem) : void
├── previous() : ListItem
├── setPrevious(ListItem) : void
└── compareTo(ListItem) : int

Node extends ListItem
├── implements all abstract methods
└── wraps Comparable value

MyLinkedList implements NodeList
├── root : ListItem
├── addItem(ListItem) : boolean
├── removeItem(ListItem) : boolean
├── performRemoval(ListItem, ListItem) : void
└── traverse(ListItem) : void
```